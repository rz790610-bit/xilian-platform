/**
 * 基础设施服务 — EventBus、Saga 编排器、Outbox、去重、配置中心
 *
 * 独立微服务入口点
 * HTTP: 3006 | gRPC: 50056
 *
 * 源文件映射（从单体迁移）:
 *   - server/services/eventBus.service.ts
 *   - server/operations/saga.orchestrator.ts
 *   - server/operations/outbox.publisher.ts
 */
...
const SERVICE_NAME = 'infra';
const HTTP_PORT = parseInt(process.env.HTTP_PORT ?? '3006', 10);
const GRPC_PORT = parseInt(process.env.GRPC_PORT ?? '50056', 10);
