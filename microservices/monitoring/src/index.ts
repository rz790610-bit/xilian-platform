/**
 * 监控告警服务 — 告警规则、诊断、可观测性聚合
 *
 * 独立微服务入口点
 * HTTP: 3005 | gRPC: 50055
 *
 * 源文件映射（从单体迁移）:
 *   - server/services/observability.service.ts
 *   - server/api/alertRule.api.ts
 */
...
const HTTP_PORT = parseInt(process.env.HTTP_PORT ?? '3005', 10);
const GRPC_PORT = parseInt(process.env.GRPC_PORT ?? '50055', 10);
...
const healthChecker = new HealthChecker(SERVICE_NAME, '1.0.0');
  ...
  // ClickHouse
  // healthChecker.register('clickhouse', clickhouseHealthCheck(chClient));
