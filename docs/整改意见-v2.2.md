...
| **配置单一源** | 全部分散在各文件 | docker.defaults.ts 已修复；剩余 **57 文件 274 处** | 100% 统一 | 需继续整改 |
| **安全漏洞 (High)** | 7 个 | **7 个**（未变） | 0 个 | 需修复 |
| **测试覆盖率** | 0% | **1.4%**（203 用例） | ≥ 60% | 需持续投入 |
| **Drizzle 迁移** | 30 张表 / 164 张定义 | **164 张表全覆盖** | 100% | **已达标** |
| **废弃文件** | 20+ 处 @deprecated | **1 处**（qdrant.ts） | 0 | 临门一脚 |
...
| **P0（待执行）** | 配置单一源收敛（剩余 57 文件） | 12h | A-05 达标 |
| **P0（待执行）** | 安全漏洞修复 | 3h | Z-02 达标 |
...
| 维度 | 当前 | P0 完成后 | Level 2 目标 |
|------|------|----------|-------------|
| ERROR 日志 | 1 处 | 1 处 | ≤ 5 ✅ |
| 配置单一源 | 57 文件 274 处 | ≤ 3 处 | 100% ✅ |
| 安全漏洞 (High) | 7 个 | 0 个 | 0 ✅ |
