...
**首次初始化即完整**：Docker volume 首次创建时，init 脚本按文件名字母序自动执行，创建全部 160 张表 + 种子数据。无需手动跑 migration。
...
| 4 | `05-base-seed.sql` | Seed | - | 基础种子数据（角色/权限/设备类型等） |
| 5 | `06-v5-seed.sql` | Seed | - | V5 种子数据（工况模板/特征定义等） |
| 6 | `07-evolution-seed.sql` | Seed | - | 进化层种子数据（仿真场景/物理方程） |

**总计：160 张表**（Pipeline 3 张表已包含在 01-base-ddl.sql 中）
