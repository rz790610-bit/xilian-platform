...
| # | 模块 | 依赖 | 当前状态 | 工作量 |
|---|------|------|---------|--------|
| P0-1 | 感知管线端到端打通（协议采集 → 状态向量） | 协议适配器 | 各层独立存在，未串联 | 2 周 |
| P0-2 | 数据质量评分集成（感知层三模块注册 + 测试） | P0-1 | 模块存在，缺测试+集成 | 1 周 |
| P0-3 | 统一编码注册表实现（4 类编码 seed data） | 无 | 设计完成，代码未实现 | 1 周 |
| P0-4 | HDE 双轨诊断端到端验证（单设备完整流程） | P0-1 | 编排器存在，未端到端验证 | 2 周 |
| P0-5 | 知识图谱基础实例化（Condition+Case 节点，3 新关系） | P0-3 | Schema 设计完成 | 1 周 |
...
| # | 模块 | 依赖 | 当前状态 | 工作量 |
|---|------|------|---------|--------|
| P1-1 | DS 融合引擎端到端验证（多源证据→融合→物理约束校验） | P0-4 | 引擎存在，未端到端验证 | 2 周 |
| P1-2 | 自动标注管线闭环（3 层 AI 降级 → 审核 → 入库） | P0-4, P0-5 | 管线存在，审核流程未闭环 | 2 周 |
| P1-3 | 进化飞轮 MVP（知识结晶 → 影子评估 → 灰度 1 轮） | P1-1, P1-2 | 各模块存在但未串联 | 3 周 |
| P1-4 | 跨设备横向对比（SHARED_COMPONENT 图谱查询） | P0-5 | 对比器存在，图谱关系未建 | 1 周 |
| P1-5 | 数字孪生实时数据接入（替换演示数据） | P0-1 | 前端完成，后端数据管道缺失 | 2 周 |
| P1-6 | 工况归一化端到端（工况识别 → 编码 → 归一化 → 标准基线） | P0-1, P0-3 | 算法存在，流程未串联 | 2 周 |
