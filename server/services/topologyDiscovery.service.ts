...
const DISCOVERABLE_SERVICES: DiscoverableService[] = [
  ...
  // ============ v5.0 认知中枢 ============
  {
    id: 'cognitive_perception',
    name: '感知层',
    type: 'engine',
    icon: '👁️',
    description: 'L1 多源数据感知与工况识别',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['data_source', 'mysql_db'],
    dependedBy: ['cognitive_engine'],
  },
  {
    id: 'cognitive_engine',
    name: '认知诊断引擎',
    type: 'agent',
    icon: '🧠',
    description: 'L2 Grok推理 + 四维诊断闭环',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['cognitive_perception', 'ollama', 'cognitive_guardrail', 'cognitive_tooling'],
    dependedBy: ['cognitive_knowledge', 'cognitive_digital_twin'],
  },
  {
    id: 'cognitive_guardrail',
    name: '安全护栏',
    type: 'plugin',
    icon: '🛡️',
    description: 'L2 规则引擎 + 阈值校验 + 告警',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['mysql_db', 'diagnosis_agent'],
    dependedBy: ['cognitive_engine'],
  },
  {
    id: 'cognitive_knowledge',
    name: '知识沉淀层',
    type: 'plugin',
    icon: '💎',
    description: 'L3 知识结晶 + 特征工程 + KG',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['cognitive_engine', 'knowledge_base', 'knowledge_graph'],
    dependedBy: ['cognitive_evolution'],
  },
  {
    ...
    type: 'engine',
    icon: '🔄',
    description: 'L4 影子评估 + 冠军挑战 + 自动迭代',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['cognitive_knowledge', 'mysql_db'],
    dependedBy: ['cognitive_perception', 'cognitive_engine'],
  },
  {
    id: 'cognitive_tooling',
    name: '工具注册中心',
    type: 'plugin',
    icon: '🔧',
    description: 'L5 工具定义 + 动态调用',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['mysql_db'],
    dependedBy: ['cognitive_engine'],
  },
  {
    ...
    type: 'engine',
    icon: '🔀',
    description: 'L6 管线定义 + DAG执行',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['mysql_db'],
    dependedBy: ['cognitive_engine'],
  },
  {
    id: 'cognitive_digital_twin',
    name: '数字孪生',
    type: 'agent',
    icon: '🏭',
    description: 'L7 设备模型 + 仿真 + 回放',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['cognitive_engine', 'mysql_db'],
  },
  {
    id: 'cognitive_data_artery',
    name: '数据动脉',
    type: 'service',
    icon: '🫀',
    description: 'L0 数据契约 + 连接器管理 + 健康巡检',
    detection: { type: 'env', envVar: 'DATABASE_URL' },
    dependsOn: ['mysql_db', 'data_source'],
    dependedBy: ['cognitive_perception'],
  },
];
