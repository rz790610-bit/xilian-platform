...
export class MqttAdapter extends BaseAdapter {
  ...
  readonly configSchema: ProtocolConfigSchema = {
    ...
    label: 'MQTT Broker',
    icon: '📡',
    description: 'MQTT/Sparkplug B — IoT 传感器实时数据流与工业消息',
    category: 'industrial',
    connectionFields: [
    ...
    advancedFields: [
      ...
      // ─── Sparkplug B（工业 IoT 标准）───
      { key: 'sparkplugEnabled', label: '启用 Sparkplug B', type: 'boolean', required: false, defaultValue: false,
        description: '启用 Sparkplug B 协议（Eclipse Tahu 标准），自动处理 NBIRTH/NDEATH/DBIRTH/DDEATH/NDATA/DDATA 消息',
        group: 'Sparkplug B' },
      { key: 'sparkplugGroupId', label: 'Group ID', type: 'string', required: false,
        placeholder: 'Factory-01',
        description: 'Sparkplug B 组标识（通常对应工厂/区域）',
        group: 'Sparkplug B' },
      { key: 'sparkplugNodeId', label: 'Edge Node ID', type: 'string', required: false,
        placeholder: 'Gateway-01',
        description: 'Sparkplug B 边缘节点标识（通常对应网关/边缘设备）',
        group: 'Sparkplug B' },
      { key: 'sparkplugDeviceIds', label: 'Device ID 列表', type: 'json', required: false,
        description: 'Sparkplug B 设备 ID 列表。示例: ["Pump-01", "Motor-02", "Valve-03"]',
        group: 'Sparkplug B' },
      { key: 'sparkplugUseCompression', label: '启用压缩', type: 'boolean', required: false, defaultValue: false,
        description: 'Sparkplug B Payload 使用 DEFLATE 压缩（减少带宽，增加 CPU 开销）',
        group: 'Sparkplug B' },
      // ─── 主题模板 ───
      { key: 'topicPrefix', label: '主题前缀', type: 'string', required: false,
        placeholder: 'factory/line1/devices',
        description: '发布/订阅的主题前缀（不含尾部 /）',
        group: '主题配置' },
      { key: 'subscribeTopics', label: '订阅主题列表', type: 'json', required: false,
      ...
      { key: 'payloadFormat', label: '消息格式', type: 'select', required: false, defaultValue: 'json', options: [
        ...
        { label: 'CSV', value: 'csv' },
        { label: '原始二进制', value: 'binary' },
      ], description: '消息负载的序列化格式',
        group: '消息格式' },
      { key: 'timestampField', label: '时间戳字段名', type: 'string', required: false,
        defaultValue: 'timestamp',
        description: 'JSON 消息中的时间戳字段名（用于数据对齐和排序）',
        group: '消息格式' },
      { key: 'timestampFormat', label: '时间戳格式', type: 'select', required: false, defaultValue: 'epoch_ms', options: [
        { label: 'Unix 毫秒 (epoch_ms)', value: 'epoch_ms' },
        { label: 'Unix 秒 (epoch_s)', value: 'epoch_s' },
        { label: 'ISO 8601', value: 'iso8601' },
      ], group: '消息格式' },
      // ─── 性能调优 ───
      { key: 'reschedulePings', label: '重调度心跳', type: 'boolean', required: false, defaultValue: true,
        description: '收到消息后重置心跳计时器',
        group: '性能调优' },
      { key: 'queueQoSZero', label: '缓存 QoS 0', type: 'boolean', required: false, defaultValue: true,
        description: '离线时是否缓存 QoS 0 消息',
        group: '性能调优' },
      { key: 'incomingStore', label: '消息缓存大小', type: 'number', required: false, defaultValue: 1000,
        description: '入站消息缓存队列大小（QoS 1/2 消息）',
        group: '性能调优' },
    ],
  };
