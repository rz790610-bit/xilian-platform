...
function localDiagnose(): string {
  ...
  lines.push(`> 模式: 本地降级（xAI API 不可用）\n`);
  ...
  for (const m of allModules.slice(0, 5)) {
    lines.push(`- **${m.label}**: ${m.completeness}% (${m.stubCount} stub, ${m.plannedCount} planned)`);
  }
  lines.push('');
...
- 使用中文回答`;
  ...
  async diagnose(userQuestion?: string): Promise<{
    ...
    try {
      ...
      return {
        diagnosis: lastAssistant?.content || localDiagnose(),
        mode: 'grok',
        timestamp: new Date(),
        toolCallCount,
      };
