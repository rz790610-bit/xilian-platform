...
export class Neo4jStorage {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      log.warn('[Neo4j] Connection failed:', error);
      throw error;
    }
  }
  ...
  async createEquipment(equipment: EquipmentNode): Promise<EquipmentNode | null> {
    ...
    try {
      ...
      log.warn('[Neo4j] Create equipment error:', error);
      return null;
    } finally {
      await session.close();
    }
  ...
  async createFault(fault: FaultNode): Promise<FaultNode | null> {
    ...
    try {
      ...
      log.warn('[Neo4j] Create fault error:', error);
      return null;
    } finally {
      await session.close();
    }
  ...
  async createSolution(solution: SolutionNode): Promise<SolutionNode | null> {
    ...
    try {
      ...
      log.warn('[Neo4j] Create solution error:', error);
      return null;
    } finally {
      await session.close();
    }
  ...
  async getNode(label: string, id: string): Promise<Record<string, unknown> | null> {
    ...
    try {
      ...
      log.warn('[Neo4j] Get node error:', error);
      return null;
    } finally {
      await session.close();
    }
  ...
  async updateNode(label: string, id: string, properties: Record<string, unknown>): Promise<boolean> {
    ...
    try {
      ...
      log.warn('[Neo4j] Update node error:', error);
      return false;
    } finally {
      await session.close();
    }
  ...
  async deleteNode(label: string, id: string): Promise<boolean> {
    ...
    try {
      ...
      log.warn('[Neo4j] Delete node error:', error);
      return false;
    } finally {
      await session.close();
    }
  ...
  async createHasPartRelation(
    ...
    try {
      ...
      log.warn('[Neo4j] Create HAS_PART relation error:', error);
      return false;
    } finally {
      await session.close();
    }
  ...
  async createCausesRelation(
    ...
    try {
      ...
      log.warn('[Neo4j] Create CAUSES relation error:', error);
      return false;
    } finally {
      await session.close();
    }
  ...
  async createSimilarToRelation(
    ...
    try {
      ...
      log.warn('[Neo4j] Create SIMILAR_TO relation error:', error);
      return false;
    } finally {
      await session.close();
    }
  ...
  async createResolvedByRelation(
    ...
    try {
      ...
      log.warn('[Neo4j] Create RESOLVED_BY relation error:', error);
      return false;
    } finally {
      await session.close();
    }
  ...
  async createAffectsRelation(
    ...
    try {
      ...
      log.warn('[Neo4j] Create AFFECTS relation error:', error);
      return false;
    } finally {
      await session.close();
    }
  ...
  async detectCommunities(nodeLabel: string = 'Fault'): Promise<CommunityResult[]> {
    ...
    try {
      ...
      log.warn('[Neo4j] Detect communities error:', error);
      return [];
    } finally {
      await session.close();
    }
  ...
  async calculatePageRank(nodeLabel: string = 'Fault'): Promise<PageRankResult[]> {
    ...
    try {
      ...
      log.warn('[Neo4j] Calculate PageRank error:', error);
      return [];
    } finally {
      await session.close();
    }
  ...
  async findSimilarByEmbedding(
    ...
    try {
      ...
      log.warn('[Neo4j] Find similar by embedding error:', error);
      return [];
    } finally {
      await session.close();
    }
  ...
  async findFaultPropagationPath(
    ...
    try {
      ...
      log.warn('[Neo4j] Find fault propagation path error:', error);
      return [];
    } finally {
      await session.close();
    }
  ...
  async findEquipmentFaultHistory(equipmentId: string): Promise<FaultNode[]> {
    ...
    try {
      ...
      log.warn('[Neo4j] Find equipment fault history error:', error);
      return [];
    } finally {
      await session.close();
    }
  ...
  async findFaultSolutions(faultId: string): Promise<SolutionNode[]> {
    ...
    try {
      ...
      log.warn('[Neo4j] Find fault solutions error:', error);
      return [];
    } finally {
      await session.close();
    }
  ...
  async searchFaults(query: string, limit: number = 20): Promise<FaultNode[]> {
    ...
    try {
      ...
      log.warn('[Neo4j] Search faults error:', error);
      return [];
    } finally {
      await session.close();
    }
  ...
  async getGraphStatistics(): Promise<{
    ...
    try {
      ...
      log.warn('[Neo4j] Get graph statistics error:', error);
      return {
        nodeCount: {},
        relationshipCount: {},
        totalNodes: 0,
